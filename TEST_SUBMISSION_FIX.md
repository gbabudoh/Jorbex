...
```bash
mongosh hireme --eval "db.testresults.find({candidateId: ObjectId('691e53af1b0408e7a633f898')}).pretty()"
```

### Check candidate status:
```bash
mongosh hireme --eval "db.candidates.findOne({email: 'godwin@egobas.com'}, {onboardingTestPassed: 1, onboardingTestScore: 1})"
```
...
```bash
mongosh hireme fix-godwin-test.js
```
