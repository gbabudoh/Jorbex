...
export default function EmployerMessagesPage() {
  ...
  const [newRecipient, setNewRecipient] = useState<{ id: string, name: string, expertise?: string } | null>(null);
  ...
  const handleSendMessage = async (isNew = false) => {
    ...
    if (isNew) {
      ...
      receiverType = 'candidate'; // Employer always sends to candidate in this search view
    } else {
      ...
      receiverType = messageType === 'inbox' 
        ...
        : (selectedMessage.receiverCandidateId ? 'candidate' : 'employer');
    }
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-base md:text-lg">
                        {messageType === 'sent' 
                          ? (selectedMessage.receiverCandidate?.name || selectedMessage.receiverEmployer?.companyName || selectedMessage.receiverEmployer?.name)
                          : (selectedMessage.senderCandidate?.name || selectedMessage.senderEmployer?.companyName || selectedMessage.senderEmployer?.name)}
                      </CardTitle>
                      <CardDescription className="text-xs md:text-sm">
                        {messageType === 'sent' 
                          ? (selectedMessage.receiverCandidate ? t('messages.candidate') : t('messages.employer'))
                          : (selectedMessage.senderCandidate ? t('messages.candidate') : t('messages.employer'))}
                      </CardDescription>
                    </div>
                  </div>
