...
export default function CreateProgrammePage() {
  ...
  if (status === 'unauthenticated') {
    router.replace('/programmes/register');
    return null;
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-950 py-10 px-4">
      <div className="max-w-2xl mx-auto">
        <button onClick={() => router.push('/programmes/dashboard')} className="flex items-center gap-2 text-gray-500 hover:text-gray-900 dark:hover:text-white mb-6 cursor-pointer text-sm">
        ...
        <div className="flex items-center gap-3 mb-8">
          {[1, 2].map(s => (
            <div key={s} className={`flex items-center gap-2 text-sm font-semibold ${step === s ? 'text-blue-600' : step > s ? 'text-gray-400' : 'text-gray-300 dark:text-gray-600'}`}>
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${step === s ? 'bg-blue-600 text-white' : step > s ? 'bg-slate-300 dark:bg-slate-700 text-gray-600 dark:text-gray-400' : 'border-2 border-slate-200 dark:border-slate-700 text-gray-400'}`}>{s}</div>
                ...
                <div className="grid gap-3">
                  {TYPES.map(t => (
                    <button
                      ...
                      className={`flex items-start gap-4 p-4 rounded-xl border-2 text-left transition-all cursor-pointer ${form.type === t.value ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-700'}`}
                    >
                ...
                <>
                  <div>
                    ...
                    {form.name && form.type && (
                      <p className="text-xs text-gray-400 mt-1">
                        Portal URL: <span className="text-blue-600 font-mono">{PORTAL_PREFIX[form.type as ProgrammeType]}{form.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}</span>
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input value={form.city} onChange={e => set('city', e.target.value)} placeholder="e.g. Lagos" className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="date" value={form.startDate} onChange={e => set('startDate', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-gray-900 dark:text-white mb-1">End Date</label>
                      <input type="date" value={form.endDate} onChange={e => set('endDate', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                  </div>

                  <button onClick={() => setStep(2)} disabled={!form.name}
                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white py-3 rounded-xl font-bold transition-colors cursor-pointer">
                    Continue →
                  </button>
                </>
              )}
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div>
                ...
                <input type="number" value={form.maxParticipants} onChange={e => set('maxParticipants', e.target.value)} placeholder="Leave blank for unlimited" className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-xl">
                ...
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${form.inviteOnly ? 'bg-blue-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                >
              ...
              <div>
                ...
                <input value={form.allowedDomains} onChange={e => set('allowedDomains', e.target.value)} placeholder="unilag.edu.ng, ui.edu.ng (comma-separated, leave blank for any)" className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
              ...
              <div>
                ...
                <textarea value={form.welcomeMessage} onChange={e => set('welcomeMessage', e.target.value)} rows={3} placeholder="Welcome message shown on the programme portal" className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
              </div>
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 pt-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={form.billingModel} onChange={e => set('billingModel', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                  ...
                  <div>
                    ...
                    <input type="number" value={form.billingAmount} onChange={e => set('billingAmount', e.target.value)} placeholder="0" className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleSubmit} disabled={loading} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white py-3 rounded-xl font-bold transition-colors cursor-pointer">
