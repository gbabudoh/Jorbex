...
export default function CorporateSchemeManagePage() {
  ...
  const [tab, setTab] = useState<'overview' | 'candidates' | 'invites'>('overview');
  ...
  useEffect(() => {
    if (session?.user?.userType === 'portal_user' && session.user.portalType === 'CORPORATE') load();
  }, [session, load]);
      ...
      <header className="border-b border-white/10 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-slate-400 text-xs">{session.user.portalName} — Corporate Portal</p>
          </div>
          {programme && (
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        {loading ? (
          ...
          <GlassCard className="p-8 text-center">
            ...
            <Link href="/programmes/corporate/dashboard" className="text-amber-400 text-sm mt-3 block">← Back to dashboard</Link>
          </GlassCard>
        ) : programme ? (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              {[
                { label: 'Candidates', value: programme.members.length },
            ...
            <div className="flex gap-1 bg-white/5 border border-white/10 rounded-xl p-1 mb-6 w-fit">
              {(['overview', 'candidates', 'invites'] as const).map((t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  className={`px-4 py-1.5 rounded-lg text-sm font-semibold capitalize transition-all ${tab === t ? 'bg-amber-500 text-white' : 'text-slate-400 hover:text-white'}`}
                >
                  {t}
                </button>
              ))}
            ...
            {tab === 'candidates' && (
              <GlassCard className="overflow-hidden">
                <div className="px-6 py-4 border-b border-white/10">
                  <h2 className="font-bold text-white">Candidates ({programme.members.length})</h2>
                </div>
                {programme.members.length === 0 ? (
                  <div className="px-6 py-12 text-center text-slate-400 text-sm">No candidates enrolled yet. Share an invite code to start building your pipeline.</div>
                ) : (
                ...
                <GlassCard className="p-5">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="Label (e.g. Tech Scheme Batch 1)"
                      className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50"
                    />
                    <button
                      onClick={createInviteCode}
                      disabled={creatingCode}
                      className="px-4 py-2.5 bg-amber-500 hover:bg-amber-400 text-white font-semibold text-sm rounded-xl transition disabled:opacity-50"
                    >
                  ...
                  {codeCreated && (
                    <div className="mt-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-3">
                      <p className="text-emerald-300 text-xs font-semibold mb-1">Code created — share with candidates:</p>
