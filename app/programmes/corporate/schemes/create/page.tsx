...
export default function CorporateCreateSchemePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error || 'Failed to create scheme'); return; }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-amber-950/30">
      <header className="border-b border-white/10 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">New Graduate Scheme</p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        <div className="mb-8">
          <h1 className="text-2xl font-black text-white">Create a Graduate Scheme</h1>
          <p className="text-slate-400 text-sm mt-1">Define your recruitment scheme and candidate intake.</p>
        </div>

        <GlassCard className="p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-semibold text-slate-300 mb-1.5">Scheme Name <span className="text-red-400">*</span></label>
              <input
                type="text"
                required
                value={form.name}
                onChange={(e) => setForm({ ...form, name: e.target.value })}
                placeholder="e.g. Technology Graduate Scheme 2026"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe the scheme, roles, and what candidates can expect..."
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition resize-none"
              />
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition"
                />
              </div>
              <div>
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-300 mb-1.5">Max Candidates</label>
              <input
                type="number"
                min="1"
            ...
            <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-4 py-3">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Candidates must use an invite code to apply</p>
              </div>
              <button
                type="button"
                onClick={() => setForm({ ...form, inviteOnly: !form.inviteOnly })}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${form.inviteOnly ? 'bg-amber-500' : 'bg-white/20'}`}
              >
            ...
            <div className="flex gap-3 pt-2">
              <Link
                href="/programmes/corporate/dashboard"
              ...
              <button
                type="submit"
                disabled={submitting}
                className="flex-1 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-white font-bold text-sm transition disabled:opacity-50"
              >
