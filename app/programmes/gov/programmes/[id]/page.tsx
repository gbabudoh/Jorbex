...
export default function GovProgrammeManagePage() {
  ...
  useEffect(() => {
    if (session?.user?.userType === 'portal_gov') load();
  }, [session, load]);
      ...
      <header className="border-b border-white/10 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-slate-400 text-xs">{session.user.portalName} — Government Portal</p>
          </div>
          {programme && (
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        {loading ? (
          ...
          <GlassCard className="p-8 text-center">
            ...
            <Link href="/programmes/gov/dashboard" className="text-blue-400 text-sm mt-3 block">← Back to dashboard</Link>
          </GlassCard>
        ) : programme ? (
          <>
            {/* Stats row */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              {[
                { label: 'Members', value: programme.members.length },
                { label: 'Invite Codes', value: programme.inviteCodes.length },
                { label: 'Country', value: programme.country || '—' },
                { label: 'Max Participants', value: programme.maxParticipants ?? 'Unlimited' },
              ].map((s) => (
            ...
            <div className="flex gap-1 bg-white/5 border border-white/10 rounded-xl p-1 mb-6 w-fit">
              {(['overview', 'members', 'invites'] as const).map((t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  className={`px-4 py-1.5 rounded-lg text-sm font-semibold capitalize transition-all ${tab === t ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}
                >
                  {t}
                </button>
              ))}
            ...
            {tab === 'overview' && (
              <GlassCard className="p-6 space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4 text-sm">
                  {programme.startDate && (
                    <div><p className="text-slate-400 text-xs mb-0.5">Start Date</p><p className="text-white">{new Date(programme.startDate).toLocaleDateString()}</p></div>
                  )}
                  {programme.endDate && (
                    <div><p className="text-slate-400 text-xs mb-0.5">End Date</p><p className="text-white">{new Date(programme.endDate).toLocaleDateString()}</p></div>
                  )}
                  ...
                  <div><p className="text-slate-400 text-xs mb-0.5">Access</p><p className="text-white">{programme.inviteOnly ? 'Invite Only' : 'Open'}</p></div>
                </div>
              </GlassCard>
            )}
            ...
            {tab === 'members' && (
              <GlassCard className="overflow-hidden">
                <div className="px-6 py-4 border-b border-white/10">
                  <h2 className="font-bold text-white">Members ({programme.members.length})</h2>
                </div>
                {programme.members.length === 0 ? (
                  <div className="px-6 py-12 text-center text-slate-400 text-sm">No members yet. Share an invite code to enrol participants.</div>
                ) : (
                        ...
                        {programme.members.map((m) => (
                          <tr key={m.id} className="border-b border-white/5 hover:bg-white/5">
                            ...
                            <td className="px-4 py-3">
                              <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${m.status === 'ACTIVE' ? 'bg-emerald-500/10 text-emerald-300' : 'bg-slate-500/10 text-slate-400'}`}>
                                {m.status}
                              </span>
                            </td>
                ...
                <GlassCard className="p-5">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="Label (e.g. Batch A 2026)"
                      className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                    />
                    <button
                      onClick={createInviteCode}
                      disabled={creatingCode}
                      className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold text-sm rounded-xl transition disabled:opacity-50"
                    >
                  ...
                  {codeCreated && (
                    <div className="mt-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-3">
                      <p className="text-emerald-300 text-xs font-semibold mb-1">Code created — copy and share:</p>
                      <p className="text-white font-mono font-bold">{codeCreated}</p>
                    </div>
                  )}
                </GlassCard>
