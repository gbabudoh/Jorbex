...
interface Programme {
  ...
  owner: { companyName: string; country?: string };
  _count: { members: number; programmeJobs: number };
  status: string;
}

export default function GovPortalPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const handleEnrol = async () => {
    if (!session) { router.push(`/login?redirect=/gov/${slug}`); return; }
  ...
  const colour = programme.primaryColour || '#10b981';
      ...
      <div className="relative py-20 px-4 text-white text-center" style={{ background: `linear-gradient(135deg, ${colour}dd, ${colour}99)` }}>
        <div className="max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-4 py-1.5 text-sm font-semibold mb-6">
            🏛️ Government Internship Programme
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-8">
          {enrolled ? (
            <div className="text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-6">You have successfully joined <strong>{programme.name}</strong>. You can now apply to positions in this programme.</p>
              <button onClick={() => router.push('/candidate/jobs')} className="w-full py-3 rounded-xl font-bold text-white transition-colors cursor-pointer" style={{ background: colour }}>
                Browse Positions
              </button>
            </div>
          ) : (
            <>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                {programme.inviteOnly ? 'An invite code is required to join.' : 'Open enrolment — no invite code needed.'}
              </p>
              ...
              {programme.inviteOnly && (
                <div className="mb-4">
                  <label className="block text-sm font-bold text-gray-900 dark:text-white mb-1">Invite Code</label>
                  <input
                    value={inviteCode}
                    onChange={e => setInviteCode(e.target.value.toUpperCase())}
                    placeholder="XXXXXXXXXX"
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white font-mono tracking-widest text-center focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
              )}

              <button onClick={handleEnrol} disabled={enrolling} className="w-full py-3 rounded-xl font-bold text-white disabled:opacity-50 transition-opacity cursor-pointer" style={{ background: colour }}>
              ...
              {!session && (
                <p className="text-center text-xs text-gray-400 mt-3">
                  New to Jorbex?{' '}
                  <button onClick={() => router.push(`/signup?type=candidate&redirect=/gov/${slug}`)} className="text-emerald-600 hover:underline cursor-pointer">Create a free account</button>
                </p>
              )}
