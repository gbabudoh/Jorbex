...
export async function POST(req: Request) {
  try {
    ...
    // Validate type
    const validTypes = ['GOVERNMENT', 'UNIVERSITY', 'CORPORATE'];
    if (!validTypes.includes(type)) {
      return NextResponse.json(
        { error: 'Invalid programme type' },
        { status: 400 },
      );
    }

    // 1. Save enquiry to database first (this is the critical part)
    const enquiry = await prisma.programmeEnquiry.create({
      data: {
        type: type as 'GOVERNMENT' | 'UNIVERSITY' | 'CORPORATE',
        institutionName,
        country,
        city: city || null,
    ...
    console.log('[Programmes enquiry saved]', { id: enquiry.id, type, institution: institutionName, email });

    // 2. Try to send emails (non-blocking — don't fail if email fails)
    ...
    try {
      ...
      console.log('[Programmes enquiry emails sent]', { type, institution: institutionName, email });
    } catch (emailErr) {
      // Log email error but don't fail the request — enquiry is already saved
      console.error('[Programmes enquiry email failed]', emailErr);
    }
